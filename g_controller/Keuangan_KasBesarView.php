...
if (isset($_SESSION['userId'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($dk == "K") {
            $saveFolder = mysql_query("INSERT INTO kasbesar(account,tgl,kas,debit,kredit,keterangan) 
                                        VALUES('$account','$tgl','$kas','0','$nominal2','$keterangan')") or die(mysql_error());
            
            // menampilkan id desc dari kasbesar
            $query = "SELECT * FROM kasbesar ORDER BY id DESC LIMIT 1";
            $mysql = mysql_query($query);
            $assoc = mysql_fetch_array($mysql);
            $id = $assoc['id'];

                // masuk ke kas kecil sebagai DEBIT
            $saveToKasKecil = mysql_query("INSERT INTO kaskecil(id_kasbesar,account,tgl,kas,debit,kredit,keterangan) 
                                            VALUES('$id','$account','$tgl','$kas','$nominal2','0','$keterangan')") or die(mysql_error());
                
            if($saveFolder AND $saveToKasKecil) {
                echo "<script>window.location.href = '/?g=Keuangan_KasBesar';</script>";
            } else {
                echo "QUERY KREDIT ERROR";
            }
        }else{
            $saveFolder = mysql_query("INSERT INTO kasbesar(account,tgl,debit,kredit,keterangan) 
                                VALUES('$account','$tgl','$nominal2','0','$keterangan')") or die(mysql_error());
            if($saveFolder) {
                echo "<script>window.location.href = '/?g=Keuangan_KasBesar';</script>";
            } else {
                echo "QUERY ERROR D, KK 2";
            }
        }
...
<head>
    <meta charset="utf-8">
    <title>GAIN Satellite Web Apps | Kas Besar</title>
...
<div class="ch-container">
    <div class="row">
        ...
        <div class="row">
                ...
                <div class="box-inner">
                    <div class="box-content">
                        <form action="" method="post">
                            ...
                            <div class="form-group has-success">
                                ...
                                <?php
                                    include_once 'database/connection.php';
                                        ...
                                        $tampil=mysql_query("SELECT * FROM account_kasbank WHERE bank = 'KB' ORDER BY id");
                            ...
                            <div class="form-group has-success">
                                <label class="control-label" for="inputSuccess1">Kas</label>
                                <select class="form-control input-sm" name="kas">
                                    <option value="" disabled selected>-Pilih Kas Kecil-</option>
            ...
            <div class="box col-md-12">
                <a href="?g=Export_KB" class="btn btn-success btn-sm" target="BLANK">Export To Excel</a>
            </div>
                    ...
                    <div class="box-content">
                        <table class="table table table-striped table-bordered bootstrap-datatable datatable responsive">
                            ...
                            <tbody>
                                <?php
                                include_once 'database/connection.php';
                                $dataDok = mysql_query("SELECT * from kasbesar ORDER BY id ASC");
                                $no = 1;
