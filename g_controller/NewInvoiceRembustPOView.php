...
<?php
    include_once 'database/connection.php';
    $id = $_GET['id'];
    $dataDok_rembust = mysql_query("SELECT * FROM invoice_rembust_po WHERE invoice='$id'");
...
<?php
    if(isset($_SESSION['userId'])){
        ...
        if(isset($_POST['submit'])){
            ...
            $ResetNumber = date('Y');
            $looping = "SELECT max(invoice_number) as maxinvoice_number, invoice_rembust FROM invoice_rembust_po WHERE year(date_invoice)='$dateInvoice_Rembust' AND invoice_rembust='$invoice_Rembust'";
            ...
            $id_value = '';
            // $value_dana= '';
            $save = mysql_query("INSERT INTO invoice_rembust_po(invoice,invoice_rembust,invoice_number,date_invoice,customer_to,address_customer,payment_to,bank,address_payment,aproved_by,position,vat) 
            VALUES('$invoice','$invoice_Rembust','$hasilkode','$dateInvoice_Rembust','$cusTo_Rembust','$addressCustomer_Rembust','$payment_Rembust','$bank','$AddressPayment_Rembust','$approved_Rembust','$position_Rembust','$nilaivat_Rembust')")or die(mysql_error());
            ...
            foreach ($order_Rembust as $value) {
                ...
                $save_order = mysql_query("INSERT INTO invoice_detail_rembust_po(idRembust, invoice_rembust, dana_rembust) VALUES ".$value_string) or die(mysql_error());
                
                // untuk merubah status menjadi invoiced(telah dibuatkan invoice) ke tabel rembust
                $update = mysql_query("UPDATE rembust SET status = 'Invoiced' WHERE invoice_rembust ="."('".$inv."') AND substr(Order_ID,1,2) = 'PO'");
                
                // untuk menjumlahkan keseluruhan dana kontrak sesuai yang di ceklis
                $updateDanaRembust = mysql_query("UPDATE invoice_rembust_po SET grand_total='$sum' WHERE invoice_rembust='$invoice_Rembust'")or die(mysql_error());
                // echo $sum;
                            ...
                            <table class="table table table-striped table-bordered bootstrap-datatable datatable responsive">
                                ...
                                <tbody>
                                        <?php
                                            ...
                                            $datarembust = mysql_query("SELECT * FROM rembust WHERE invoice_rembust = '$invoice_rembust' AND NOT RembustID = 'Non Rembust' AND substr(Order_ID,1,2)= 'PO' Order By id ASC");
                                            ...
                                            <tr>
                                                ...
                                                <?php
                                                    if ($fetchDataRembust['status'] == "Invoiced") {
                                                        ...
                                                        echo "<input type='text' name='dana_rembust[]' required class='form-control uang input-sm' value='".$fetchDataRembust['Dana_Rembust']."'>";
                                                    }
                                                ?>
                                                </td>
                                                <td>
