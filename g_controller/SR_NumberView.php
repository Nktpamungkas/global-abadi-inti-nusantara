...
<?php
    include_once 'database/connection.php';
    $id = $_GET['id'];
    $dataSN = mysql_query("SELECT * FROM sr_number WHERE Order_ID='$id'");
...
if(isset($_POST['submit'])){
    ...
    $save = mysql_query("INSERT INTO sr_number(Order_ID,sr_number)VALUES('$id','$sr_number')");

    $saveFolder = mysql_query("UPDATE tbl_order SET pelunasan = 'Pelunasan', status_srnumber = '2' WHERE id = '$id'");
    if($saveFolder) {
        echo "<script>window.location.href = '/?g=FinishOrder';</script>";	
        $pesanError = 'Berhasil menginput serial number, silahkan kembali.';
    } else {
        // header("Location: ?");
        $pesanError = 'Jika terjadi kesalahan, mohon untuk merefresh halaman tersebut. Hub. Development! Kesalahan Syntax Query.';
        echo 'Jika terjadi kesalahan, mohon untuk merefresh halaman tersebut. Hub. Development! Kesalahan Syntax Query.';
    }
    ...
    $Update = mysql_query("UPDATE sr_number SET sr_number = '$sr_number' WHERE Order_ID = '$id'");
    if($Update){
        echo "<script>window.location.href = '/?g=FinishOrder';</script>";	
